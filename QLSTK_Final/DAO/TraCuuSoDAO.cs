...
namespace DAO
{
    ...
    public class TraCuuSoDAO
    {
        ...
        public DataTable loadSo()
        {
            ...
            try
            {
                string query = "select Masotietkiem as N'Mã sổ',Masotietkiem as N'Mã loại',Tenkhachhang as N'Khách hàng',Sodu as N'Số dư' from SOTIETKIEM";
        ...
        public DataTable timKiemSo(string maSo)
        {
            ...
            try
            {
                string query = "select Masotietkiem as N'Mã sổ',Masotietkiem as N'Mã loại',Tenkhachhang as N'Khách hàng',Sodu as N'Số dư' from SOTIETKIEM where Masotietkiem = '" + maSo + "'";
                data = DataProvider.Instance.ExecuteQuery(query);
                addSTT(data, 0);
            }catch(Exception ex)
            {
